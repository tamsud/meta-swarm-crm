...
async def main() -> None:
    ...
    async with AsyncSessionLocal() as db:
        roles_created, roles_skipped = await seed_custom_roles(db)
        print(f"Roles:         {roles_created} created, {roles_skipped} skipped")

        accounts_created, accounts_skipped, accounts = await seed_accounts(db)
        print(f"Accounts:      {accounts_created} created, {accounts_skipped} skipped")

        contacts_created, contacts_skipped, contacts = await seed_contacts(db, accounts)
        print(f"Contacts:      {contacts_created} created, {contacts_skipped} skipped")

        opps_created, opps_skipped = await seed_opportunities(db, accounts, contacts)
        print(f"Opportunities: {opps_created} created, {opps_skipped} skipped")

        emails_created, emails_skipped = await seed_mock_emails(db)
        print(f"Mock Emails:   {emails_created} created, {emails_skipped} skipped")
